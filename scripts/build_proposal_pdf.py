...
def build():
    ...
    flow = [
        ...
        Paragraph(
            "面向 CSV、JSONL 等常见记录型数据，构建 MoonBit 原生的数据契约与质量校验工具，支持契约解析、规则检查、数据画像、差异分析和 CLI 使用方式，帮助开发者在数据进入业务流程前发现重复值、缺失值、非法枚举、数值异常和跨字段不一致问题。",
            body,
        ),
    ...
    table = Table(
        [
            ...
            ["工程交付", "README、测试、示例数据、CLI、CI、发布准备"],
        ],
    ...
    flow.extend(
        [
            table,
            Spacer(1, 5),
            Paragraph("四、仓库链接", heading),
            Paragraph(
                "GitHub 仓库：https://github.com/Wchwch777/MoonVerity<br/>"
                "GitLink 仓库：https://gitlink.org.cn/Wchwch/moonverity",
                body,
            ),
            Paragraph("五、技术特点", heading),
            Paragraph(
                "MoonBit 原生实现，结构清晰，适合作为生态基础设施复用；采用“核心库 + CLI + 示例仓库”交付形态，强调规则显式、结果可解释、边界可扩展，既便于比赛评审，也便于后续维护。",
                body,
            ),
            Paragraph("六、预期成果", heading),
            Paragraph(
                "形成公开 GitHub / GitLink 仓库、可运行 MoonBit 包与命令行工具、完整 README、测试、示例和 CI，并具备后续扩展与发布准备基础。",
                body,
            ),
        ]
    )
